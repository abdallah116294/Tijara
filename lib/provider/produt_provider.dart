...
class ProductProvider with ChangeNotifier {
  ...
  List<ProductModel> findByCategory({
    ...
    List<ProductModel> ctgList = products
        .where((element) => element.prodcutCategory
            .toLowerCase()
  ...
  Future<List<ProductModel>> fetchProducts() async {
    final productDB = await FirebaseFirestore.instance.collection("products");
    try {
      products.clear();
      await productDB.orderBy("createdAt",descending: false).get().then((productsSnapshot) {
        for (var element in productsSnapshot.docs) {
          products.insert(0, ProductModel.fromFirsestore(element));
        }
      });
      notifyListeners();
      return products;
  ...
  Stream<List<ProductModel>> fetchproductstStream() {
